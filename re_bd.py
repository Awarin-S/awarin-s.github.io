...
def downC(constI, charN):
    ...
    os.makedirs(f'characters/{charN}', exist_ok=True)
    save_path = f'characters/{charN}/{constI}.png'
    with open(save_path, 'wb') as out:
        out.write(content)
...
def downS(skillN, charN):
    ...
    os.makedirs(f'characters/{charN}', exist_ok=True)
    save_path = f'characters/{charN}/{skillN}.png'
    with open(save_path, 'wb') as out:
        out.write(content)
...
def down(char_list_api_en, i, url):
    ...
    if url != 'https://enka.network/ui/UI_Gacha_AvatarImg_':
        ...
        os.makedirs(f'characters/{char_name}', exist_ok=True)
    else:
        # Создание папки, если её нет
        char_name = char_list_api_en['categories']['outfits']['characterName'][i]
        os.makedirs(f'characters/{char_name}', exist_ok=True)
    ...
    if url == 'https://enka.network/ui/UI_AvatarIcon_':
        save_path = f'characters/{char_name}/{char_name}_icon.png'
        compressed_path = f'characters/{char_name}/{char_name}_icon.webp'
    elif url == 'https://enka.network/ui/UI_Gacha_AvatarImg_':
        save_path = f'characters/{char_name}/{char_name}_gacha.webp'
    else:
        save_path = f'characters/{char_name}/{char_name}_namecard.png'
        compressed_path = f'characters/{char_name}/{char_name}_namecard.webp'
